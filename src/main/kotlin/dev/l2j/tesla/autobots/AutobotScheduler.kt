...
import dev.l2j.tesla.autobots.models.ScheduledSpawnInfo
import dev.l2j.tesla.autobots.utils.Klock
...
internal object AutobotScheduler {
    ...
    init {
        Klock.start("scheduledBots", 1000, 30000,
            CoScopes.generalScope, action = {
    ...
    private fun handleScheduledBotSpawn(dateTimeNow: LocalDateTime, it: ScheduledSpawnInfo) {
        val loginDateTime = LocalDateTime.parse("${dateTimeNow.year}-${if (dateTimeNow.monthValue < 10) "0${dateTimeNow.monthValue}" else dateTimeNow.monthValue}-${if (dateTimeNow.dayOfMonth < 10) "0${dateTimeNow.dayOfMonth}" else dateTimeNow.dayOfMonth} ${it.loginTime}",
            formatter
        )
        val logoutDateTime = LocalDateTime.parse("${dateTimeNow.year}-${if (dateTimeNow.monthValue < 10) "0${dateTimeNow.monthValue}" else dateTimeNow.monthValue}-${if (dateTimeNow.dayOfMonth < 10) "0${dateTimeNow.dayOfMonth}" else dateTimeNow.dayOfMonth} ${it.logoutTime}",
            formatter
        )
