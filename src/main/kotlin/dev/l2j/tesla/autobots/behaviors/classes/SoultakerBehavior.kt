...
import dev.l2j.tesla.autobots.behaviors.preferences.skills.SoultakerSkillPreferences
...
internal class SoultakerBehavior(player: Player, combatPreferences: CombatPreferences) : CombatBehavior(player, combatPreferences),
    RequiresMiscItem,
    PetOwner {

    override var skillPreferences: SkillPreferences =
        SoultakerSkillPreferences()
    ...
    override val summonInfo: Pair<BotSkill, MiscItem?> = Pair(BotSkill(1129) { _, _, target -> target != null && target.isDead },
        MiscItem({ 1459 }, 50, 10)
    )
