...
import dev.l2j.tesla.autobots.behaviors.preferences.PetOwnerPreferences
            ...
            if (bot.hasServitor()) {
                ...
                if (prefs.petAssists) {
                    ...
                    if (servitor.target.isAutoAttackable(servitor)) {
                        ...
                        if (servitor.target != null && !GeoEngine.getInstance().canSeeTarget(servitor, servitor.target)) servitor.ai.setIntention(
                            IntentionType.FOLLOW, this)
                    }
                } else {
                    val servitor = bot.summon as Servitor
