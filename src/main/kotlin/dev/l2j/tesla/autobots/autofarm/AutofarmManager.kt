...
import dev.l2j.tesla.autobots.extensions.getCombatBehaviorForClass
import dev.l2j.tesla.autobots.extensions.getSocialBehavior
import dev.l2j.tesla.autobots.extensions.isClassSupported
...
object AutofarmManager {
    ...
    fun startFarm(player: Player){
        ...
        socialBehaviors[player.objectId] =
            SocialBehavior(
                player,
                SocialPreferences(TownAction.None)
            )
                ...
                ps.executeQuery().use { rs ->
                    ...
                    while(rs.next()) {
                        ...
                        val combatPrefs = if(jsonCombatPrefs.isNullOrEmpty()) {
                            ...
                            mapper.readValue(jsonCombatPrefs, supportedCombatPrefs.getOrDefault(
                                ClassId.values().first { it.id == player.activeClass}) { DefaultCombatPreferences() }.invoke().javaClass)
                        }
